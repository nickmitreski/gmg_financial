...
export async function POST(request: Request) {
  try {
    ...
    const { data: updated } = await supabase.from('articles').update({ status: 'published', published_at: new Date().toISOString() }).eq('id', id).select('slug').single()
    if (!updated) return NextResponse.json({ error: 'Failed to publish' }, { status: 400 })
